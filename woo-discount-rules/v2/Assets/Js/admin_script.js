...
jQuery(document).ready(function ($) {
    ...
    // Forza la presenza dell'opzione "Fixed price" nella tendina BXGY
(function ensureBxgyFixedPriceOption(){
    const label = (wdr_data && wdr_data.localization_data && (wdr_data.localization_data.buyx_getx_fixed_price || wdr_data.localization_data.fixed_price_label)) || 'Fixed price';
    $('.awdr-bogo-discount-type').each(function(){
        if ($(this).find('option[value="fixed_price"]').length === 0) {
            // Inserisci dopo "Fixed discount" se presente, altrimenti in coda
    ...
    $(document).on('change', '.awdr-bogo-discount-type', function () {
        ...
        switch (get_bogo_free_type) {
            ...
            case 'fixed_price':
                $(this).parent('.' + get_bogo_free_type_parent).siblings('.' + get_bogo_free_type_siblings).show();
                $(this).parent('.' + get_bogo_free_type_parent).siblings('.' + get_bogo_free_type_siblings).find('.wdr_desc_text').text(wdr_data.localization_data.buyx_getx_fixed_price || wdr_data.localization_data.buyx_getx_value);
                break;
            case 'free_product':
            default:
